  ...
  while (isRunning) {
    if (!this->isPaused) {
      ...
      if (!chunk || chunk->pcmSize == 0) {
        ...
        // not very clean but bell::dsp is not always available
        for (int16_t* pcm = (int16_t*)chunk->pcmData; (uint8_t*)pcm < chunk->pcmData + chunk->pcmSize; pcm++) {
            *pcm = (*pcm * volume) / UINT16_MAX;
        }
